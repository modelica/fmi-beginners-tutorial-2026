...
DYMOLA_STATIC double DymolaTimerCounterSinceNCB(double*d,int i);
DYMOLA_STATIC double MyCPUTime(void);
DYMOLA_STATIC double MyCPUTime(void) {
	static int i=0;
	static double d;
	double res= DymolaTimerCounterSinceNCB(&d,i);
	i=1;
	return res;
}
#if defined(DYMOLA_DSPACE)
...
DYMOLA_STATIC void AssertModelicaFNon(int b, const char* bs, const char* x, int qi, int* bad) {
	if (!b) {
		char buf[DYNDiagnosticBufL];
		if (qi < 0) {
			sprintf(buf, "  " DYNDiagnosticStrL "\n  Condition failed initially, delay check: " DYNDiagnosticStrL, x, bs);
		} else {
			sprintf(buf, "  " DYNDiagnosticStrL "\n  Failed condition: " DYNDiagnosticStrL, x, bs);
		}
		if (qi < 0) {
...
#define positiveDymola(x, sx) ((x<=0 && (AssertModelicaF(0, sx, "Interval Clock must be positive"),1)),x)
...
#define BreakSectionTMDef(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      ...
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,double*Qold_,int thetaIter,double*mixedModeThetaP,double*mixedModeTheta2P,double*deltaTheta2P);
...
#define BreakSectionTMStart(nr) DYMOLA_STATIC int equationsJ_##nr(int *idemand_, int *icall_, \
      ...
      double* F_, double* Y_, double* W_,  double QZ_[], \
      double duser_[], int iuser_[], void*cuser_[],struct DYNInstanceData*did_ DYNOPTArgCSD, \
	   int *QiErr,double*Qold_,int thetaIter,double*mixedModeThetaP,double*mixedModeTheta2P,double*deltaTheta2P) {double mixedModeTheta,mixedModeTheta2,deltaTheta2;int thetaNr;\
mixedModeTheta=*mixedModeThetaP;mixedModeTheta2=*mixedModeTheta2P;deltaTheta2=*deltaTheta2P;
...
#define BreakSectionTMCall(nr) \
   { BreakSectionTMDef(nr) switch(equationsJ_##nr(idemand_, icall_, \
      ...
      F_, Y_, W_, QZ_, \
      duser_, iuser_, cuser_,did_ DYNOPTCallCSD, \
      QiErr,Qold_,thetaIter,&mixedModeTheta,&mixedModeTheta2,&deltaTheta2)) { case 1:goto finish;case 2:goto leave;default:;} \
	}
...
#define BreakSectionTMEnd() \
	   *mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2;return 0;finish:*mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2; return 1;leave:*mixedModeThetaP=mixedModeTheta;*mixedModeTheta2P=mixedModeTheta2;*deltaTheta2P=deltaTheta2;return 2;}
...
#define BreakSectionJacNLData int QInfRev,int*QDYNBadStart,
#define BreakSectionJacNLDataS QInfRev,QDYNBadStart,
...
#define BreakSectionJacNLDef(nr) BreakSectionJacCommonDef(nr) BreakSectionJacNLData BreakSectionDenseJacobianData(Jacobian__, int) );
#define BreakSectionJacSparseNLDef(nr) BreakSectionJacCommonDef(nr) BreakSectionJacNLData BreakSectionSparseJacobianData(Jacobian__, int*) );
...
#define BreakSectionJacNLCallNewName(nr,name) \
	BreakSectionJacCommonCallNewName(nr,name,BreakSectionJacNLDataS BreakSectionDenseJacobianData(name, ))
#define BreakSectionJacSparseNLCallNewName(nr,name) \
	BreakSectionJacCommonCallNewName(nr,name,BreakSectionJacNLDataS BreakSectionSparseJacobianData(name, ))
...
#define BreakSectionJacNLPCallNewName(nr, name) \
	BreakSectionJacCommonPCallNewName(nr,name,BreakSectionJacNLDataS BreakSectionDenseJacobianData(name, ))
#define BreakSectionJacSparseNLPCallNewName(nr, name) \
	BreakSectionJacCommonPCallNewName(nr,name,BreakSectionJacNLDataS BreakSectionSparseJacobianData(name, ))
...
#define BreakSectionJacNLCallNew(nr) BreakSectionJacNLCallNewName(nr, Jacobian__)
#define BreakSectionJacSparseNLCallNew(nr) BreakSectionJacSparseNLCallNewName(nr, Jacobian__)
...
#define BreakSectionJacNLEnd() BreakSectionJacEnd()
#define BreakSectionJacSparseNLEnd() BreakSectionJacEnd()
...
#define BreakSectionJacNLStart(nr) BreakSectionJacCommonStart(nr) BreakSectionJacNLData BreakSectionDenseJacobianData(Jacobian__, int) ) {
#define BreakSectionJacSparseNLStart(nr) BreakSectionJacCommonStart(nr) BreakSectionJacNLData BreakSectionSparseJacobianData(Jacobian__, int*) ) {
...
#define StepSizeCalculations(iv)\
    ...
    if (previousTime_<-1e29) {previousTime_=DYNTime;currentStepSize_=0;}\
    ...
    if (DYNFindEvent && StepSize == 0 && DYNTime > LastTime && LastTime>-1e29) {\
...
#if defined(RT)
LIBDS_API double DymolaTimerCounterSinceNCB(double*d,int i) {
	if (i==0) {
		double x=0;
		if (d) *d=x;
		return x;
...
LIBDS_API double DymolaTimerCounterSince(double* d, int i) {
	return DymolaTimerCounterSinceNCB(d, i);
}
#else // defined(RT)
#include <time.h>
...
double DymolaTimerCounterSinceNCB(double* d, int i) {
	if (i == 0) {
		double x = myclock();
		if (d) *d = x;
		return x;
...
__declspec(dllexport) 
	...
	a.arraySizes = 0;
	a.SetDymolaDirPointers = SetDymolaDirPointers;
#ifdef GODESS
	{
