...
#define RTFOUND          +1
#define CLOSERT          +3
#define ACCURATE_DAE_CROSSING_REDO	+999
...
void *IDACreate(SUNContext sunctx)
{
  ...
  DS extensions
  -----------------------*/
  IDA_mem->ida_stepgfun = NULL;
  IDA_mem->ida_monitorfun = NULL;
  IDA_mem->ida_atdaeeventfun = NULL;
  IDA_mem->ida_accurate_dae_crossing = 0;
  IDA_mem->ida_next_time_event = DBL_MAX;
  IDA_mem->ida_stateerrormonitorfun = NULL;
             ...
             N_Vector yret, N_Vector ypret, int itask)
{
    ...
    if (IDA_mem->ida_nrtfn > 0) {
      ...
      if ( SUNRabs(IDA_mem->ida_tn - IDA_mem->ida_tretlast) > troundoff ) {
        ...
        if (ier == IDA_SUCCESS) {     /* no root found */
          if (itask == IDA_ONE_STEP && IDA_mem->ida_next_time_event < IDA_mem->ida_tn) {
              IDA_mem->ida_thi = IDA_mem->ida_next_time_event;
              (void) IDAGetSolution(IDA_mem, IDA_mem->ida_thi, yret, ypret);
              IDA_mem->ida_irfnd = 1;
              IDA_mem->ida_tretlast = *tret = IDA_mem->ida_thi;
              return(IDA_TIME_ROOT_RETURN);
          }
          IDA_mem->ida_irfnd = 0;
  ...
  for(;;) {
    ...
    if (itask == IDA_ONE_STEP && IDA_mem->ida_next_time_event < IDA_mem->ida_tn) {
        ...
        (void) IDAGetSolution(IDA_mem, IDA_mem->ida_thi, yret, ypret);
        IDA_mem->ida_irfnd = 1;
        istate = IDA_TIME_ROOT_RETURN;
        IDA_mem->ida_tretlast = *tret = IDA_mem->ida_thi;
        break;
    }

	if (IDA_mem->ida_stepgfun && (IDA_mem->ida_stepgfun)(IDA_mem->ida_user_data)) {
		if (IDA_mem->ida_taskc == IDA_ONE_STEP) {
			IDA_mem->ida_thi = IDA_mem->ida_tn;
		} else if (IDA_mem->ida_taskc == IDA_NORMAL) {
			if ((IDA_mem->ida_toutc - IDA_mem->ida_tn)*IDA_mem->ida_hh >= ZERO)
				IDA_mem->ida_thi = IDA_mem->ida_tn;
			else
				IDA_mem->ida_thi = IDA_mem->ida_toutc;
		}
...
static int IDANls(IDAMem IDA_mem)
{
  ...
  if (IDA_mem->ida_stateerrormonitorfun) {
      for (i = 0; i < nnf_inc; ++i)
          (*IDA_mem->ida_stateerrormonitorfun)(IDA_mem->ida_user_data, IDA_mem->ida_tn - IDA_mem->ida_hh, IDA_mem->ida_hh, IDA_mem->ida_kk, 2, 0, NULL, NULL, 0.0, 0.0);
  }
                        ...
                        realtype *err_k, realtype *err_km1)
{
  ...
  *err_k = IDA_mem->ida_sigma[IDA_mem->ida_kk] * enorm_k;
  ...
  if (IDA_mem->ida_stateerrormonitorfun)
      (*IDA_mem->ida_stateerrormonitorfun)(IDA_mem->ida_user_data, IDA_mem->ida_tn - IDA_mem->ida_hh, IDA_mem->ida_hh, IDA_mem->ida_kk, ck * enorm_k > ONE, N_VGetLength(IDA_mem->ida_ewt),
          N_VGetArrayPointer(IDA_mem->ida_ewt), N_VGetArrayPointer(IDA_mem->ida_ee), ck * enorm_k, ck);
...
static void IDACompleteStep(IDAMem IDA_mem, realtype err_k, realtype err_km1)
{
  ...
  if (IDA_mem->ida_monitorfun != NULL) {
    IDA_mem->ida_monitorfun((void*) IDA_mem, IDA_mem->ida_user_data);
  }
#endif
}
...
static int IDARcheck1(IDAMem IDA_mem)
{
  ...
  if (IDA_mem->ida_atdaeeventfun)
	IDA_mem->ida_atdaeeventfun(IDA_mem->ida_user_data, 0);
...
static int IDARcheck3(IDAMem IDA_mem)
{
  ...
  if (IDA_mem->ida_taskc == IDA_ONE_STEP)
      IDA_mem->ida_thi = (IDA_mem->ida_next_time_event < IDA_mem->ida_tn ? IDA_mem->ida_next_time_event : IDA_mem->ida_tn);
  ...
  } while (ier == ACCURATE_DAE_CROSSING_REDO);
...
static int IDARootfind(IDAMem IDA_mem)
{
  ...
  /* If no sign change was found, reset trout and grout.  Then return
     IDA_SUCCESS if no zero was found,...  */
  if (!sgnchg) {
  ...
  if (IDA_mem->ida_atdaeeventfun && !IDA_mem->ida_accurate_dae_crossing) {
	if (IDA_mem->ida_atdaeeventfun(IDA_mem->ida_user_data, 1)) {
		IDA_mem->ida_accurate_dae_crossing = 1;
		return(ACCURATE_DAE_CROSSING_REDO);
	}
  }

  /* ... or set iroots and return RTFOUND. */
  if (!sgnchg) {
    for (i = 0; i < IDA_mem->ida_nrtfn; i++) {
      IDA_mem->ida_iroots[i] = 0;
      if(!IDA_mem->ida_gactive[i]) continue;
