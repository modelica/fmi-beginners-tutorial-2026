...
SUNDIALS_EXPORT int cvLsATimes(void* cvode_mem, N_Vector v, N_Vector z);
SUNDIALS_EXPORT int cvLsPSetup(void* cvode_mem);
SUNDIALS_EXPORT int cvLsPSolve(void* cvode_mem, N_Vector r, N_Vector z,
               realtype tol, int lr);

/* Difference quotient approximation for Jac times vector */
SUNDIALS_EXPORT int cvLsDQJtimes(N_Vector v, N_Vector Jv, realtype t,
                 N_Vector y, N_Vector fy, void *data,
                 N_Vector work);

/* Difference-quotient Jacobian approximation routines */
SUNDIALS_EXPORT int cvLsDQJac(realtype t, N_Vector y, N_Vector fy, SUNMatrix Jac,
              void *data, N_Vector tmp1, N_Vector tmp2, N_Vector tmp3);
SUNDIALS_EXPORT int cvLsDenseDQJac(realtype t, N_Vector y, N_Vector fy,
                   SUNMatrix Jac, CVodeMem cv_mem, N_Vector tmp1);
SUNDIALS_EXPORT int cvLsBandDQJac(realtype t, N_Vector y, N_Vector fy,
                  SUNMatrix Jac, CVodeMem cv_mem, N_Vector tmp1,
                  N_Vector tmp2);

/* Generic linit/lsetup/lsolve/lfree interface routines for CVode to call */
SUNDIALS_EXPORT int cvLsInitialize(CVodeMem cv_mem);
SUNDIALS_EXPORT int cvLsSetup(CVodeMem cv_mem, int convfail, N_Vector ypred,
              N_Vector fpred, booleantype *jcurPtr,
              N_Vector vtemp1, N_Vector vtemp2, N_Vector vtemp3);
SUNDIALS_EXPORT int cvLsSolve(CVodeMem cv_mem, N_Vector b, N_Vector weight,
              N_Vector ycur, N_Vector fcur);
SUNDIALS_EXPORT int cvLsFree(CVodeMem cv_mem);

/* Auxilliary functions */
SUNDIALS_EXPORT int cvLsInitializeCounters(CVLsMem cvls_mem);
SUNDIALS_EXPORT int cvLs_AccessLMem(void* cvode_mem, const char* fname,
