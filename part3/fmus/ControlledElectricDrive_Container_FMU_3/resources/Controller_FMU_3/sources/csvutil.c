...
/* Returns true if line contains any character not expected in a data row. */
{
	...
	if (line == 0) return 0;
	while (*line != '\0') {
		if (strchr(datachar, *line) == NULL) {
			if ((line[0] == 'n' || line[0] == 'N') && (line[1] == 'a' || line[1] == 'A') && (line[2] == 'n' || line[2] == 'N')) {
				line += 3;
				if (line[0] == '(' && line[1] == ')') line += 2;
				else if (line[0] == '(' && line[1] == 'i' && line[2] == 'n' && line[3] == 'd' && line[4] == ')') line += 5;
				continue;
			}
			return 1;
		}
    ...
    if (!LineEmpty(line)) {
		...
		if (i!=sz) {
			...
			if (!hasTimedOut) {
				...
				if (n_rows != data_dim1 || n_columns != data_dim2) ModelicaFormatError("Mismatched sizes. Should have %d=%d and %d=%d", n_rows, data_dim1, n_columns, data_dim2);
				}
			return;
		}
    }
...
int WriteCSVmatrixInternal(const char*fname, const double*values, size_t rows, size_t columns, 
