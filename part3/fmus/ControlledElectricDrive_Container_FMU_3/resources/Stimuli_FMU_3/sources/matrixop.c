...
#include <limits.h>
#include <float.h>
#define SUPPORT_MALLOC_FOR_NON_TEMP 1
#if SUPPORT_MALLOC_FOR_NON_TEMP
#include <stdlib.h>
#endif
#include <string.h>
...
DYMOLA_STATIC void EnsureMarkInitializedFunction() {
	...
	;
		...
		threadData->m_stringmark=threadData->m_extra.m_startstringmark=calloc(numStringSimple,1);
		threadData->m_extra.m_endsimplestring=threadData->m_stringmark+ numStringSimple;
	}
}
DYMOLA_STATIC void EnsureMarkFree(struct DYN_ThreadData*threadData) {
	if (threadData && 0!=threadData->m_extra.m_endreal) {
		...
		free((void*)threadData->m_start.place.Stringbuffer);
...
DYMOLA_STATIC struct DYNFunctionData_ DYNGetFunctionData() {
...
DYMOLA_STATIC Real* RealNonTemp(SizeType r) {
	Real* d = 0;
#if SUPPORT_MALLOC_FOR_NON_TEMP
	if (currentMarkNon.place.Realbuffer + r >= EndRealbufferNon) {
		d = (Real*)malloc(r * sizeof(Real));
		Assert(!!d, "Out of memory for reals\nIt could due to too large matrices, infinite recursion, or uninitialized variables.\n");
	} else
#endif
	{
		d = currentMarkNon.place.Realbuffer;
#if SUPPORT_MALLOC_FOR_NON_TEMP
		Assert(currentMarkNon.place.Realbuffer + r < EndRealbufferNon, "Out of memory for reals\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\n");
#else
		Assert(currentMarkNon.place.Realbuffer + r < EndRealbufferNon, "Out of memory for reals\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\nYou have modified defines to disable malloc, change that or change the memory size by setting the variable Advanced.Translation.RealBufferSize.");
#endif
...
DYMOLA_STATIC Integer* IntegerNonTemp(SizeType r) {
	Integer* i = 0;
#if SUPPORT_MALLOC_FOR_NON_TEMP
	if (currentMarkNon.place.Integerbuffer + r >= EndIntegerbufferNon) {
		i = (Integer*)malloc(r * sizeof(Integer));
		Assert(!!i, "Out of memory for integers\nIt could due to too large matrices, infinite recursion, or uninitialized variables.\n");
	} else
#endif
	{
		i = currentMarkNon.place.Integerbuffer;
#if SUPPORT_MALLOC_FOR_NON_TEMP
		Assert(currentMarkNon.place.Integerbuffer + r < EndIntegerbufferNon, "Out of memory for integers\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\n");
#else
		Assert(currentMarkNon.place.Integerbuffer + r < EndIntegerbufferNon, "Out of memory for integers\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\nYou have modified defines to disable malloc, change that or increase the size of 'Integerbuffer' in dymola/source/matrixop.h.");
#endif
...
DYMOLA_STATIC SizeType* SizeNonTemp(SizeType r) {
	SizeType* s = 0;
#if SUPPORT_MALLOC_FOR_NON_TEMP
	if (currentMarkNon.place.Sizebuffer + r >= EndSizebufferNon) {
		s = (SizeType*)malloc(r * sizeof(SizeType));
		Assert(!!s, "Out of memory for array dimensions\nIt could due to too many matrices, infinite recursion, or uninitialized variables.\n");
	} else
#endif
	{
		s = currentMarkNon.place.Sizebuffer;
#if SUPPORT_MALLOC_FOR_NON_TEMP
		Assert(currentMarkNon.place.Sizebuffer + r < EndSizebufferNon, "Out of memory for array dimensions\nIt is likely due to too many matrices, infinite recursion, or uninitialized variables.\n");
#else
		Assert(currentMarkNon.place.Sizebuffer + r < EndSizebufferNon, "Out of memory for array dimensions\nIt is likely due to too many matrices, infinite recursion, or uninitialized variables.\nYou have modified defines to disable malloc, change that or increase the size of 'Sizebuffer' in dymola/source/matrixop.h.");
#endif
...
DYMOLA_STATIC String* StringNonTemp(SizeType r) {
	String* s = 0;
#if SUPPORT_MALLOC_FOR_NON_TEMP
	if (currentMarkNon.place.Stringbuffer + r >= EndStringbufferNon) {
		s = (String*)malloc(r * sizeof(String));
		Assert(!!s, "Out of memory for strings\nIt could due to too large matrices, infinite recursion, or uninitialized variables.\n");
	} else
#endif
	{
		s = currentMarkNon.place.Stringbuffer;
#if SUPPORT_MALLOC_FOR_NON_TEMP
		Assert(currentMarkNon.place.Stringbuffer + r < EndStringbufferNon, "Out of memory for strings\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\n");
#else
		Assert(currentMarkNon.place.Stringbuffer + r < EndStringbufferNon, "Out of memory for strings\nIt is likely due to too large matrices, infinite recursion, or uninitialized variables.\nYou have modified defines to disable malloc, change that or increase the size of 'Stringbuffer' in dymola/source/matrixop.h.");
#endif
...
DYMOLA_STATIC void RealFromSub(RealArray to,SizeType to_offset,SizeType to_dim,
  ...
  case Range:case RangeRange:
    {
      Integer start,stride=1,i,from_newdim,to_newdim;
      start=ap->u.range[0];
	...
	/* Pass 2. Determine size of output */
	{
		...
		for (; !over;) {
			...
			case Vector:
			{
				...
				SizeType i;
				Assert(v.ndims == 1, "Array subscripting: only vectors (and scalars) allowed as indices");
...
DYMOLA_STATIC void RealPutSub(const RealArray ain,RealArray out,...) {
  RealArray a;
  va_list ap;
  SizeType out_dim=0,a_dim=0,trailingsize=1;
  a=RealCopy(ain);
  /* Pass 1. Determine nr. dimensions of output */
  {
    Integer over=0;
...
DYMOLA_STATIC Integer real2integer(Real x)
{
	const Real result = floor(x);
	Assert(
		  !(isnan(result) || result < INT_MIN || result > INT_MAX)
		, "Converting Real to Integer failed; the value is out of range.");
...
DYMOLA_STATIC StringArray DYNJreadMATmatrices_M(const char*fil, struct DYNFunctionData_*DYNStackData_) {
	...
	for (; ret == 0;) {
		...
		ret = amatGetMatrix(&afile, &amatrix);
		if (ret <= 1) {
			char*s1 = StringAllocate((SizeType)strlen(amatrix.name));
...
DYMOLA_STATIC String ConvertToNonTempString(String s) {
	if (s>=startstringmark && s<Endsimplestring) {
		...
		Assert(startstringmarkNon+len+1<EndsimplestringNon,"Room to allocate string");
...
DYMOLA_STATIC String StringAdd(String a,String b) {
	char*ret=StringAllocate((SizeType)(strlen(a)+strlen(b)));
	strcpy(ret,a);
	strcat(ret,b);
	return ret;
}
...
#define MODELICA_NORETURNATTR
#endif
/* gcc 4.8.3 (at least) does not allow the noreturn attribute (MODELICA_NORETURNATTR) in function definitions, so add explicit declarations first to be able to use it */
MODELICA_NORETURN DYMOLA_STATIC void ModelicaError(const char* string) MODELICA_NORETURNATTR;
MODELICA_NORETURN DYMOLA_STATIC void ModelicaError(const char *string) {
	DymosimErr((string));
}
MODELICA_NORETURN DYMOLA_STATIC void ModelicaFormatError(const char* string, ...) MODELICA_NORETURNATTR;
MODELICA_NORETURN DYMOLA_STATIC void ModelicaFormatError(const char *string,...) {
	char buf[4000];
	va_list ap;
	va_start(ap,string);
...
DYMOLA_STATIC char* ModelicaAllocateString(size_t len) {
	char*s=StringAllocate((SizeType)len);
	s[len]='\0';
	return s;
}
...
DYMOLA_STATIC char* ModelicaDuplicateString(const char*str) {
	...
	if (str == 0) ModelicaFormatError("ModelicaDuplicateString with null-pointer");
		...
		s = StringAllocate((SizeType)(strlen(str) + 1));
		strcpy(s, str);
	}
	return s;
}
...
DYMOLA_STATIC DYN_UNUSED_ATTR RealArray RealNonTempCopy(RealArray x) {
	RealArray res;
	SizeType i, nel;
	res.ndims = x.ndims;
	res.dims = SizeNonTemp(x.ndims);
	for (i = 0; i < res.ndims; ++i) res.dims[i] = x.dims[i];
	nel = RealNrElements(res);
	res.data = RealNonTemp(nel);
	for (i = 0; i < nel; i++) {
		res.data[i] = x.data[i];
	}
	return res;
...
DYMOLA_STATIC DYN_UNUSED_ATTR void RealNonTempSetSize(RealArray* x, SizeType ndims, SizeType* dims) {
	SizeType resize = 0, i = 0, nel = 1;
	if (!(x && dims))
		return;
	if (x->ndims != ndims) {
		x->ndims = ndims;
		x->dims = SizeNonTemp(ndims);
		resize = 1;
	}
	for (i = 0; i < x->ndims; ++i) {
		if (x->dims[i] != dims[i]) {
	...
	if (resize) {
		x->data = RealNonTemp(nel);
	}
}
DYMOLA_STATIC DYN_UNUSED_ATTR void RealNonTempCopyData(RealArray* x, const Real* data, SizeType nbrData) {
	SizeType nel = RealNrElements(*x);
	if (!data || nel != nbrData)
			return;
	memcpy(x->data, data, nbrData * sizeof(Real));
}

DYMOLA_STATIC DYN_UNUSED_ATTR IntegerArray IntegerNonTempCopy(IntegerArray x) {
	IntegerArray res;
	SizeType i, nel;
	res.ndims = x.ndims;
	res.dims = SizeNonTemp(x.ndims);
	for (i = 0; i < res.ndims; ++i) res.dims[i] = x.dims[i];
	nel = IntegerNrElements(res);
	res.data = IntegerNonTemp(nel);
	for (i = 0; i < nel; i++) {
		res.data[i] = x.data[i];
	}
	return res;
...
DYMOLA_STATIC DYN_UNUSED_ATTR void IntegerNonTempCopyData(IntegerArray* x, const Integer* data, SizeType nbrData) {
	SizeType nel = IntegerNrElements(*x);
	if (!data || nel != nbrData)
		return;
	memcpy(x->data, data, nbrData * sizeof(Integer));
}
...
DYMOLA_STATIC DYN_UNUSED_ATTR void IntegerNonTempSetSize(IntegerArray* x, SizeType ndims, SizeType* dims) {
	...
	if (resize) {
		x->data = IntegerNonTemp(nel);
	}
}
DYMOLA_STATIC DYN_UNUSED_ATTR StringArray StringNonTempCopy(StringArray x) {
	StringArray res;
	SizeType i, nel;
	res.ndims = x.ndims;
	res.dims = SizeNonTemp(x.ndims);
	for (i = 0; i < res.ndims; ++i) res.dims[i] = x.dims[i];
	nel = StringNrElements(res);
	res.data = StringNonTemp(nel);
	for (i = 0; i < nel; i++) {
		res.data[i] = ConvertToNonTempString(x.data[i]);
	}
	return res;
}
