...
/*    GlobalError_ = 0; */  /* In order to handle errors at events. */ 
      ...
      if (Init_) {
		...
		if (DymolaOneIteration_ == 0 || DymolaOneIteration_ == 2) {
			for (i1_ = 0; i1_ < NSparse_; i1_++)
				did_->sparse_data[i1_] = NULL;
		}

		for (i1_ = 0; i1_ < NUM_ITERATION_VARIABLES; i1_++)
			did_->dae_iteration_variable_nominals[i1_] = 0.0;
