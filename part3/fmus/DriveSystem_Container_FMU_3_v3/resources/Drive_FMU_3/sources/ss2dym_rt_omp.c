...
extern void DymosimEnsureDLLLoaded(const char*const*toLoad, void***handles, int load, const char* fmuDLLpath, const char*const*);
void**handles=0;
		...
		if (getBasicIDymosimStruct()->mInlineIntegration==1 && getBasicDDymosimStruct()->mDymolaFixedStepSize<=0) {
            if (ssIsVariableStepSolver(S) || (strcmp(ssGetSolverName(S),"FixedStepDiscrete")!=0 &&
                ...
                ssSetErrorStatus(S, "Error encountered in Dymola block: \n"
                                    ...
                                    "To use other solvers, set Advanced.Translation.InlineAsDiscrete = true in Dymola.\n"
        ...
        InitTime_=*time;
		{
			...
			if (handles==0) {
				if (dllLibraryPath != 0 && dllLibraryPath[0] != 0) {
					DymosimEnsureDLLLoaded(dllLibraryPath, &handles, 1, 0, 0);
				}
			}
#endif
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	*/
    ...
    if (sizeof(real_T) != sizeof(double))
      DymosimErr("Incompatible size of real_T and double");
#else
