   ...
   /* Read matrices from a file. */
      ...
      while (1) { 
         ...
         if (Matrices[nMatrices] == 0) {
           DymosimErr("Simulator could not allocate space for matrix.");
           return 1;
         }
         ...
         if (nMatrices >= MaxMatrices) {
           DymosimErr("Simulator is not able to read all matrices from data file.");
           amatGetClose(&file);
           return 2;
         }
      }
...
static AmatrixP CreateDoubleMatrix(int nx, int ny) { 
  AmatrixP m;
    ...
    m = amatNew();
    if (m == 0) {
      DymosimErr("Simulator is unfortunately out of memory.");
      return 0;
      }
    m->nrow = nx;
    m->ncol = ny;
...
/*  Linear interpolation in one dimension: y=f(x). 
  ...
  if ((Mxval != 1 && Nxval != 1) || (Myval != 1 && Nyval != 1)) {
    DymosimErr("Arguments to Interpolate1 must be vectors.");
    return 0.0; /* Not reached */
    }
  ...
  if (Kxval != Kyval) {
    DymosimErr("The x and y vectors must have the same length in Interpolate1.");  
    return 0.0; /* Not reached */
    }
  else if (x <= xval[0])
    return (yval[0]);
...
/*  Linear interpolation in n dimensions: y=f(x1, x2, ..., xn). 
  ...
  if ((Mx != 1 && Nx != 1) || (Mdim != 1 && Ndim != 1)) {
    DymosimErr("Arguments to Interpolate must be vectors.");
    return 0.0; /* Not reached */
    } 
  Kx = Mx > 1 ? Mx : Nx;
  ...
  if (Kx != Kdim) {
    DymosimErr("The size of the argument vector is not the same as\n"
  ...
  for (i=0;i<Kdim;i++) {
    if (dim[i] != floor(dim[i])) {
      DymosimErr("The dimension vector of a table must only contain "
                   "integers.");
      return 0.0; /* Not reached */
      }
  ...
  if (Kxval != dimsum) {
    DymosimErr("The size of the grid vector is not consistent with\n"
                 "  information in the dimension vector of a table.");
    return 0.0; /* Not reached */
    } 
  if (Kyval != dimprod) {
    DymosimErr("The size of the table value vector is not\n"
