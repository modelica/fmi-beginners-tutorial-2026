...
SUNDIALS_EXPORT void *CVodeCreate(int lmm, SUNContext sunctx)
{
  ...
  cv_mem->cv_stateerrormonitorfun = NULL;
...
static int cvNls(CVodeMem cv_mem, int nflag)
{
  ...
  if (cv_mem->cv_stateerrormonitorfun) {
      for (i = 0; i < nnf_inc; ++i)
          (*cv_mem->cv_stateerrormonitorfun)(cv_mem->cv_user_data, cv_mem->cv_tn - cv_mem->cv_h, cv_mem->cv_h, cv_mem->cv_q, 2, 0, NULL, NULL, 0.0, 0.0);
  }
                         ...
                         int *nefPtr, realtype *dsmPtr)
{
  ...
  if (cv_mem->cv_stateerrormonitorfun)
      (*cv_mem->cv_stateerrormonitorfun)(cv_mem->cv_user_data, saved_t, cv_mem->cv_h, cv_mem->cv_q, dsm > ONE, N_VGetLength(cv_mem->cv_ewt),
          N_VGetArrayPointer(cv_mem->cv_ewt), N_VGetArrayPointer(cv_mem->cv_acor), dsm, cv_mem->cv_tq[2]);
