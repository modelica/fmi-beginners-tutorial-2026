	...
	if(nRequiredIntermediateVariables){
		...
		for (i = 0; i < nRequiredIntermediateVariables; ++i) {
			...
			case fmiInput:
				...
				break;
			default:
				util_logger(comp, comp->instanceName, fmi3Warning, loggBadCall, "%s: requiredIntermediateVariables[%d] = neither input nor output not supported", label, i);
			}
		}
		comp->intermediateUpdate = intermediateUpdate;
		comp->nRequiredIntermediateInputs = !comp->useInputSmoother ? nrInput : 0; /* It is not allowed to set inputs with intermediateUpdate if input smoothing is enabled */
												...
												FMIBoolean loggingOn)
{
	...
	if (functions && functions->stepFinished != NULL) {
...
{	
#if (FMI_VERSION >= FMI_3)
	static FMIString label = "fmi3Terminate";
#elif (FMI_VERSION >= FMI_2)
	...
	if (comp->mStatus == modelInstantiated) {
		util_logger(comp, comp->instanceName, FMIError, loggBadCall, "%s: FMU not initialized", label);
		return util_error(comp);
	}
	...
	if(comp->QiErr == 0 && comp->terminationByModel == FMIFalse){
		...
		if (comp->did) {
			...
			dsblock_did(&terminal, &comp->icall, &comp->time, comp->states, comp->DAEMode && comp->DAEDerivatives ? comp->DAEDerivatives : 0,
			...
			dsblock_(&terminal, &comp->icall, &comp->time, comp->states, comp->DAEMode && comp->DAEDerivatives ? comp->DAEDerivatives : 0,             
...
DYMOLA_STATIC FMIStatus fmiResetSlave_(FMIComponent c)
{
	...
	switch (comp->mStatus) {
		...
		case modelTerminated:
			...
			/*clear all work data and reinitialize did, can cause isses with nonlinear sovlers when reset otherwise.*/
			if (comp->did) {
			...
			if (comp->DAEDerivatives) {
				for (i = 0; i < comp->nStates; ++i)
					comp->DAEDerivatives[i] = comp->derivatives[i];
			}
													...
													const  FMIReal value[])
{
	...
	if (comp->useInputSmoother) {
		util_logger(comp, comp->instanceName, FMIError, loggBadCall, "%s is not supported with input smoother", label);
		return util_error(comp);
	}
	if(	!comp->ipData){
					 ...
					 FMIBoolean newStep)
{
	...
	if(!comp->istruct->mInlineIntegration){
		...
		FMIBoolean willStopAtSynchronizedTimeEvent = FMIFalse;

		if (comp->useInputSmoother) {
			const FMIStatus status = InputSmoother_Apply(comp, currentCommunicationPoint, endStepTime);
			if (status != FMIOK)
				return status;
		}
		...
		/* Time events can be predicted/handled outside integration steps while state/step events must be
		   ...
		   each step. */
		while (comp->time < endStepTime || !finalTimeEventCalled
				&& (endStepTime == comp->dstruct->mNextTimeEvent || willStopAtSynchronizedTimeEvent && comp->dstruct->mNextTimeEvent < endStepTime)) {
			...
			if (comp->time < comp->dstruct->mNextTimeEvent) {
				...
				willStopAtSynchronizedTimeEvent = FMIFalse;
				if (comp->dstruct->mNextTimeEvent < endStepTime && comp->dstruct->mNextTimeEvent <= comp->nextResultSampleTime
						&& comp->dstruct->mNextTimeEvent >= endStepTime - 2.0*DBL_EPSILON*Dymola_max(Dymola_abs(comp->dstruct->mNextTimeEvent), Dymola_abs(endStepTime))) {
					/* Time event slightly before communication point. This causes problems for SUNDIALS.
					   We are not allowed to move the communication point, but we may slightly delay the time event. */
					willStopAtSynchronizedTimeEvent = FMITrue;
					nextTime = endStepTime;
					if (comp->nextResultSampleTime < endStepTime)
						/* Result sample also slightly before communication point. Delay also result sample */
						comp->nextResultSampleTime = endStepTime;
				}
				/* integrate */
				/* A bit messy*/
				switch(
#if (FMI_VERSION >= FMI_3)
					(comp->earlyReturnAllowed && comp->eventModeUsed)?integration_step_return_at_event(comp, &nextTime, comp->eventModeUsed, comp->earlyReturnAllowed, &tRetEarly, eventEncountered, endStepTime):
#endif
					integration_step(comp, &nextTime, endStepTime)) {
					case integrationOK:
#if (FMI_VERSION >= FMI_3)
				...
				if (willStopAtSynchronizedTimeEvent && comp->dstruct->mNextTimeEvent >= endStepTime)
					willStopAtSynchronizedTimeEvent = FMIFalse; /* Time event removed by state event */
				if(comp->storeResult && nextTime == comp->nextResultSampleTime && !callEventUpdate && nextTime != comp->dstruct->mNextTimeEvent && !willStopAtSynchronizedTimeEvent){
					/*if event sampling will be handled at the event instead*/
					CS_RESULT_SAMPLE(FMIFalse);
				}
			} else {
				const FMIReal eventTime = willStopAtSynchronizedTimeEvent && comp->dstruct->mNextTimeEvent < endStepTime
					&& comp->dstruct->mNextTimeEvent < comp->time && comp->time == endStepTime ? endStepTime : comp->dstruct->mNextTimeEvent;
					/* Is this delayed time event? The tests are needed as a time event may have been revealed or removed by a state event inside integration_step */
				willStopAtSynchronizedTimeEvent = FMIFalse; /* To be safe, should not be needed */
				fmiSetTime_(c, eventTime);
...
#if (FMI_VERSION >= FMI_3)
					if ((status == fmi3Discard) && terminateSimulation) {
		...
		if (comp->useInputSmoother) {
			const FMIStatus status = InputSmoother_UpdateOldInputs(comp, endStepTime);
			if (status != FMIOK)
				return status;
		}
...

{
	...
	while (comp->time < endStepTime || endStepTime == comp->dstruct->mNextTimeEvent){
		if (comp->time < comp->dstruct->mNextTimeEvent) {
			...
			/* integrate */
			switch(integration_step_return_at_event(comp, &nextTime, FMITrue, FMITrue, endTime, &dummyEvent, endStepTime)) {
				case integrationOK:
					break;
				case integrationTerminate:
					status = FMIDiscard;
