 ...
 * estimate, usually leading to fewer f-evaluations, Jacobian computations, and rejeceted steps.
 */
...
DYMOLA_STATIC FMIStatus InputSmoother_CheckSettings(Component* comp)
{
	if (comp->useInputSmoother) {
#if (FMI_VERSION != FMI_2)
		util_logger(comp, comp->instanceName, FMIFatal, loggBadCall, "Input smoother can only be used with FMI 2");
		return FMIFatal;
#endif
...
#else
			util_logger(comp, comp->instanceName, FMIFatal, loggBadCall, "Predictor compensation is only supported for FMI 2.");
			return FMIFatal;
#endif
			comp->icall = 0;
