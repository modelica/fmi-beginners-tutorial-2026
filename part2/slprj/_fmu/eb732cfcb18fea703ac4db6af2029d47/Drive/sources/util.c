...
DYMOLA_STATIC FMIStatus checkSizes(FMIComponent c, size_t nVr, size_t nv, FMIString funcName, FMIString getSet){
	if(nVr != nv){
		Component *comp = (Component*) c;
		util_logger(comp, comp->instanceName, FMIError, loggBadCall,
			"%s%s: nValuereferences (%d) and nValues (%d) does not match", getSet, funcName, nVr, nv);
		return FMIError;
	}
	return FMIOK;
}
