...
LIBDS_API int amatPutRreal2(AmatPutFile* file, const real       row[], int shouldFlush);
LIBDS_API int amatPutRdouble2(AmatPutFile* file, const doublereal row[], int shouldFlush);
...
/* Write row to file
   ...
   -> shouldFlush: Ensures that the file is flushed so that it can be read incrementally
      <- RETURN: = 0 : no error occurs.
                 else: an error occured; the error message is stored
                       as (static) string in global variable "amatError".
   */
