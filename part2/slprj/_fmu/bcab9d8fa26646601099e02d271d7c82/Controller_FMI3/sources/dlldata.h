...
struct SimulatorFunctions {
	...
	int (*SetIntegerDynamicArray)(struct DymosimSimulator* dymosimData, const unsigned int vr, const int values[], const int nbrVals);
	int (*GetIntegerDynamicArray)(struct DymosimSimulator* dymosimData, const unsigned int vr, int values[], const int nbrVals);
	int (*EnterConfigurationMode)(struct DymosimSimulator* instance);
	int (*ExitConfigurationMode)(struct DymosimSimulator* instance);
};
#include "dsblock.h"
...
struct DymosimSimulator { 
	...
	struct {
		...
		integer nxm, nx, nu, ny, nzm, nz, nw, nf[9], nfsim[10], nfm, nfmsim, nsig, 
	...
	struct {
		...
		integer nkev, ngridp, nstep, nfev, nhev, njev, nte, nue, nse, noe, kmax, 
			nee, nfev_excl_Jac_evals, nfev_predictor_compensation;
	} aadsm5_;
	...
	struct fmiInputExtr {
		...
		double t0;
		double preExtrTime;
		int useInputSmoother;
		int usePredictorCompensation;
		int* inputUsesSmoother;
		double* inputForSmoother;
	} fmiInputExtr;
	...
	struct DynArraySize {
		...
		size_t nbrInt;
		size_t nbrStr;
		double* dim; //simplifes set and get code too keep it doulbe in libfmi.
		int dimSetable;
	} dynArraySize;
