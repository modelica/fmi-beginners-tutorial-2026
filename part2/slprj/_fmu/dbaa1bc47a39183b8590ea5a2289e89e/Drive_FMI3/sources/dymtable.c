...
DYMOLA_STATIC double dymTableInit(double nipoIn, double ipoTypeIn, const char *tableName,
   ...
   /* Action according to table type */
      if ( tType == TABLE_FROM_FIELD ) {
         ...
         */
            DymosimErr("unexpected error in function \"dymTableInit1a\".");
            goto ERROR_;
#else
...
#ifndef NDEBUG
#define tableGetX(i) ((!(i>=1&&i<nt1)&&(DymosimErr("X"),0)),tableGet(i+1,1))
#define tableGetY(i) ((!(i>=1&&i<nt2)&&(DymosimErr("Y"),0)),tableGet(1,i+1))
#define tableGetZ(i,j) ((!(i>=1&&i<nt1)&&(DymosimErr("Z(x)"),0)),(!(j>=1&&j<nt2)&&(DymosimErr("Z(y)"),0)),tableGet(i+1,j+1))
#else
...
/* ERROR EXIT */
L710:
	DymosimErr("Number of x values <= 1");
	return 1;
L720:
	DymosimErr("Number of y values <= 1");
    return 1;
/* FORMAT STATEMENTS */
} /* itplbv_ */
