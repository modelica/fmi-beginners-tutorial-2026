...
DYMOLA_STATIC FMIStatus FMIUpdateBeforeGet(FMIComponent c, const FMIString label, const FMIValueReference valueReferences[], size_t nVr, size_t nVal){
	...
	if (nVr > nVal) {
		ERROR_RETURN_CHECK(checkSizes(comp, nVr, nVal, label, get));
	}
...
extern const size_t*const globalValueReferencePositions[15];
...
DYMOLA_STATIC FMIStatus fmi3Get(FMIComponent c, typePointer *tp, fmi3Types type, const FMIValueReference* vr, size_t nvr, size_t nval){
	...
	for (i = 0; i < nvr; i++) {
		...
		size_t array_size = 1, j;
		size_t firstPos = ix;
		const FMIReal* source = NULL;
		...
		if (0xffffffff == r) {
			...
			ix = 0xfffffffe - r;
			source = comp->crossingFunctions;
			firstPos = ix;
		}
		else if (globalValueReferencePositions && globalValueReferencePositions[iCat])
		{
			array_size = globalValueReferencePositions[iCat][ix + 1] - globalValueReferencePositions[iCat][ix];
			firstPos = globalValueReferencePositions[iCat][ix];
		}
		if (curSize + array_size > nval)
		{
			util_logger(comp, comp->instanceName, FMIError, loggBadCall,
				"fmiGet%s: nValuereferences (%d) and nValues (%d) does not match", sfuncVer, curSize + array_size, nval);
			return FMIError;
		}
		...
		if (!source) {
			...
			case dsOutput:
				...
				break;
			case dsAux:
				source = comp->auxiliary;
				break;
			case dsState:
				source = comp->states;
				break;
			case dsDerivative:
				source = comp->derivatives;
				break;
			case dsParameter2:
			case dsParameter:
				source = comp->parameters;
				break;
			case dsInput: 
			case dsInput2:
				source = comp->inputs;
		...
		if (source) {
			...
			case fmiTypeFloat64:
				for (j = 0; j < array_size; j++) {
					tp->rp[j+curSize] = source[firstPos + j];

					util_logger(comp, comp->instanceName, FMIOK, loggFuncCall,
						"fmiGet%s: #%u# = %g", sfuncVer, r, source[firstPos + j]);
				}
				break;
			case fmiTypeInt32:
				for (j = 0; j < array_size; j++) {
					tp->ip[j+curSize] = (fmi3Int32)source[firstPos + j];
			...
			case fmiTypeInt64:
				for (j = 0; j < array_size; j++) {
					tp->i64p[j+curSize] = (fmi3Int64)source[firstPos + j];
			...
			case fmiTypeBoolean:
				for (j = 0; j < array_size; j++) {
					tp->bp[j+curSize] = (fmi3Boolean)source[firstPos + j];
...
DYMOLA_STATIC FMIStatus fmi3Set(FMIComponent c, ctypePointer *tp, fmi3Types type, const FMIValueReference* vr, size_t nvr, size_t nval){
	Component* comp = (Component*) c;
	size_t i, curSize = 0;
	...
	for (i = 0; i < nvr; i++) {
		...
		{
			util_logger(comp, comp->instanceName, FMIError, loggBadCall,
				"fmiSet%s: nValuereferences (%d) and nValues (%d) does not match", sfuncVer, curSize + array_size, nval);
			return FMIError;
		}
		...
		/* Note that dest==NULL is not an error */
		if (dest) {
			switch (type)
			{
			case fmiTypeFloat64:
				for (j = 0; j < array_size; j++)
				{
					if (dest[firstPos + j] != tp->rp[j+curSize]) {
						dest[firstPos + j] = tp->rp[j+curSize];
						modified = FMITrue;
						switch (iCat) {
							case dsInput:
								if (comp->isCoSim) {

									integration_sync_extrapolation_input(comp, firstPos + j);
			...
			case fmiTypeInt32:
				for (j = 0; j < array_size; j++)
					dest[firstPos + j] = (FMIReal) tp->ip[j+curSize];
				break;
			case fmiTypeInt64:
				for (j = 0; j < array_size; j++)
					dest[firstPos + j] = (FMIReal)tp->i64p[j+curSize];
				break;
			case fmiTypeBoolean:
				for (j = 0; j < array_size; j++)
					dest[firstPos + j] = (FMIReal)tp->bp[j+curSize];
				break;
			default:
				return FMIFatal;
			}
 		...
 		if (allowed == FMIFalse) {
		util_logger(comp, comp->instanceName, FMIError, loggBadCall,
			       "fmiSet%s: may not change #%u# at this "
			       "stage",sfuncVer, r);
			return util_error(comp);
		}
		if (dest)
		{
			util_logger(comp, comp->instanceName, FMIOK, loggFuncCall,
			"fmiSet%s: #%u# = %g",sfuncVer, r, (double) dest[firstPos]);
		}
		curSize += array_size;
	}
#if 0
	ERROR_RETURN_CHECK(checkSizes(comp, nvr, nvals, sfuncVer));
	...
	for (i = 0; i < nvr; i++) {
		...
		case fmiTypeFloat64:
			...
			break;
		case fmiTypeInt32:
			value = (FMIReal)tp->ip[i];
			break;
		case fmiTypeInt64:
			value = (FMIReal)tp->i64p[i];
			break;
		case fmiTypeBoolean:
			value = (FMIReal) tp->bp[i];
			break;
		default:
		...
		case dsInput:
			...
			break;
		case dsState:
			if (comp->states[ix] != value) { 
				comp->states[ix] = value; 
				modified = FMITrue;
			}
			break;
		case dsAux:
		...
		case dsDerivative:
			if (comp->mStatus == modelInstantiated || comp->mStatus == modelInitializationMode){
					if (comp->derivatives[ix] != value) {
						comp->derivatives[ix] = value;
						modified = FMITrue;
					}
			} else {
				allowed = FMIFalse;
		...
		case dsOutput:
			if (comp->mStatus == modelInstantiated || comp->mStatus == modelInitializationMode){
					if (comp->outputs[ix] != value) {
						comp->outputs[ix] = value;
						modified = FMITrue;
					}
			} else {
				allowed = FMIFalse;
			...
			break;
		case dsDState:
			if (comp->dStates[ix] != value) {
				comp->dStates[ix] = value;
				modified = FMITrue;
			}
			break;
		case dsPrevious: 
			if (comp->dStates[ix] != value) {
				comp->dStates[ix] = value; /*Set the state is correct*/
                modified = FMITrue;
			}
			break;
		case dsParameter2:
...
DYMOLA_STATIC FMIStatus fmi3SetString_(FMIComponent c, const FMIValueReference vr[], size_t nvr, const FMIString  value[], size_t nvals)
{
	...
	ERROR_RETURN_CHECK(checkSizes(comp, nvr, nvals, label, set))
	for (i = 0; i < nvr; i++) {
		size_t len;
		const FMIValueReference r = vr[i];
		int ix = FMI_INDEX(r);
