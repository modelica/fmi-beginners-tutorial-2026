							   ...
							   FMIString category, FMIString message, ...)
{
	...
	if (comp->loggMessage == NULL)
		return;
#elif (FMI_VERSION >= FMI_2)
	if (comp->functions == NULL || comp->functions->logger == NULL)
		return;
#endif
...
DYMOLA_STATIC int util_check_flag(void *flagvalue, char *funcname, int opt, Component* comp)
{
	...
	/* Check if flag < 0 */
		...
		if (*errflag < 0) {
			char * CVodeFlagName;
			const FMIBoolean IDA = (FMIBoolean) GetAdditionalFlags(27); 
#ifdef INCLUDE_SUNDIALS_IDA
			if (IDA)
				CVodeFlagName = IDAGetReturnFlagName(*errflag);
			else
#endif
				CVodeFlagName = CVodeGetReturnFlagName(*errflag);
			util_logger(comp, comp->instanceName, FMIError, loggSundials,
...
DYMOLA_STATIC int util_refresh_cache(Component* comp, int idemand, const char* label, FMIBoolean* iterationConverged)
{
	...
	if (comp->did) {
		...
		dsblock_did(&idemand, &comp->icall, &comp->time, comp->states, comp->DAEMode && comp->DAEDerivatives ? comp->DAEDerivatives : 0,
		...
		dsblock_(&idemand, &comp->icall, &comp->time, comp->states, comp->DAEMode && comp->DAEDerivatives ? comp->DAEDerivatives : 0,             
...
DYMOLA_STATIC FMIStatus util_initialize_model(FMIComponent c, FMIBoolean toleranceControlled, FMIReal relativeTolerance, FMIBoolean complete)
{
	...
	if (!comp->isCoSim && comp->DAEMode) {
		status = FMIFatal;
		util_logger(comp, comp->instanceName, FMIFatal, loggBadCall, "DAE mode cannot be used with model exchange");
		return status;
	}
	if (!comp->isCoSim && comp->useInputSmoother) {
		comp->useInputSmoother = FMIFalse;
		comp->usePredictorCompensation = FMIFalse;
		util_logger(comp, comp->instanceName, FMIWarning, loggUndef, "Disabling input smoother for model exchange");
	}
...
#ifndef ONLY_INCLUDE_INLINE_INTEGRATION
		const FMIStatus status = InputSmoother_CheckSettings(comp);
		if (status != FMIOK)
			return status;
#else
		return FMIFatal;
...
#ifndef ONLY_INCLUDE_INLINE_INTEGRATION
	if (comp->useInputSmoother) {
		if (InputSmoother_Initialize(comp) != FMIOK)
			return util_error(comp);
	}
#endif
	return FMIOK;
...
DYMOLA_STATIC FMIStatus checkSizes(FMIComponent c, const FMIValueReference * vr, size_t nVr, size_t nv, FMIString funcName, FMIString getSet){
	if(nVr != nv){
		...
		for(i = 0; i< nVr; ++i){
			...
			}else if (aiCat == fmiDynArray) {
				nbrElements+=(size_t) comp->dynArraySize.dim[FMI_INDEX(vr[i])];
			}else {
				nbrElements++;
			}
		}
